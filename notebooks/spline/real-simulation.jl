...
fake = @where(sim_neigh, :index1 .== idx[1], :index2 .== idx[2]).fake[1]
cc_c = [];

# ╔═╡ b72554f4-a283-11eb-38cd-bb04ca251638
for sim in fake
	push!(cc_c, sum(crosscor.(c1, sim, -20, 20, 0.5)))
end

p2 = plot(sum(cc_c), legend=false, title="Complex model - Original cells (40 sims)", ylabel="Counts", xlabel="Time (ms)")
xticks!([0, 20, 40, 60, 80], string.([-20, -10, 0, 10, 20]))


# ╔═╡ 8d803f84-a2b0-11eb-0910-b7a7ab94c76d
savefig(p1, plotsdir("logbook", "21-04", "original-cells-real.png"))
savefig(p2, plotsdir("logbook", "21-04", "original-cells-complex-20sims.png"))
...
plot(cc_s, legend=false, title="Simple model", ylabel="Counts", xlabel="Time (ms)")
xticks!([0, 20, 40, 60, 80], string.([-20, -10, 0, 10, 20]))
