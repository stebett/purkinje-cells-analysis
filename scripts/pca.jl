...
plotlyjs()
N = slice(data.t, data.lift, around=[-200, 200], convolution = true, normalization=true, average=false)
N = dropnancols(N)
...
N = slice(data.t, data[neigh, "lift"], convolution=true, normalization=false)
N = dropnancols(N)
M = fit(PCA, N, maxoutdim=3)
